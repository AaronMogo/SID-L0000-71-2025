...
namespace ApiClientLibrary.Services
{
    public class F3_Pruebas
    {
        ...
        public async Task<HttpResponseMessage> AgregaResultadoPruebaAsync(
            ResultadoPruebaDTO resultado, string expediente, string muestra)
        {
            var json = JsonSerializer.Serialize(resultado);
            var content = new StringContent(json, Encoding.UTF8, "application/json");

            // Agregar parámetros en la URL
            var url = $"AgregaResultadoPrueba?expediente={expediente}&muestra={muestra}";
